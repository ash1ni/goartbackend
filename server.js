...
app.post('/artist-media', async (req, res) => {
  ...
  try {
    const result = await pool.query('INSERT INTO artist_media (artist_id, media_id, position) VALUES ($1, $2, $3) RETURNING *', [artist_id, media_id, position]);
    res.status(201).json(result.rows[0]);
  } catch (error) {
...
app.delete('/artist-media/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query('DELETE FROM artist_media WHERE id = $1 RETURNING *', [id]);
...
app.get('/collection_media/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('SELECT * FROM collection_media WHERE id = $1', [id]);
    if (result.rows.length === 0) {
...
app.delete('/collection_media/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('DELETE FROM collection_media WHERE id = $1 RETURNING *', [id]);
    if (result.rows.length === 0) {
      res.status(404).json({ error: 'Collection media not found' });
    } else {
